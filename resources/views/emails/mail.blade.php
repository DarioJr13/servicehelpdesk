 ...
 */
$tinyMCEPattern = '#'.url('/').'/storage/tinymce_attachments/.*\" alt#';
preg_match_all($tinyMCEPattern, $data, $matches);

foreach($matches[0] as $match) {
    $break = explode('tinymce_attachments', $match);
    $embeded = $message->embed(storage_path('app/public/tinymce_attachments').DIRECTORY_SEPARATOR.str_replace('" alt', '', $break[1]));
    $data = str_replace($match, $embeded.'" alt', $data);
}
?>
@endif
{!! $data !!}
