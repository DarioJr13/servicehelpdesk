      ...
      <ul class="products-list product-list-in-card">
        <li class="item" v-for="activity in notificationData" :key="activity.id">
          ...
          <div class="product-info fw-400" style="padding-top: 1rem;">
            ...
            <router-link v-if="activity.by === 'System'" class="product-title" :to="getUrl(activity.url)" target="_blank">
              {{activity.by + ' ' + activity.message}}
            </router-link>

            <router-link v-else class="product-title  fw-400"  :to="activity.url.replace(basePath()+'/panel','')" target="_blank">
              {{ activity.requester.changed_by_first_name + ' ' + activity.requester.changed_by_last_name + ' ' + activity.message}}
            </router-link>
...
import axios from 'axios';
    ...
    name : 'dashboard-recent-activities',
      ...
      getUrl(url) {
        ...
        return url.includes('panel') ? url.replace(this.basePath()+'/panel','') : url.replace(this.basePath(),'');
      },
