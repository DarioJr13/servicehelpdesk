        ...
        templates: {
          ...
          title: function(createElement, row) {
            ...
            if(self.isCalledFromAdmin) {
              ...
              return createElement('a', { attrs: { href: self.basePath()+'/panel/thread/' + row.id }},(row.title ? row.title : " ")+ '('+row.thread_count+')');

            } else {

              return createElement('router-link', { attrs: { to: '/thread/' + row.id }},(row.title ? row.title : " ")+ '('+row.thread_count+')');
            }
          },
          ...
          user: function(createElement, row) {
             ...
             if(self.isCalledFromAdmin) {
              ...
              return createElement('a', { attrs: { href: self.basePath()+'/panel/user/' + row.from.id }}, row.from.full_name ? row.from.full_name : row.from.user_name);

             } else {

                return createElement('router-link', { attrs: { to: '/user/' + row.from.id }}, row.from.full_name ? row.from.full_name : row.from.user_name);
             }
          },
          ...
          ticket_number: function(createElement, row) {
            ...
            if(self.isCalledFromAdmin) {
              ...
              return createElement('a', { attrs: { href: self.basePath()+'/panel/thread/' + row.id }}, '#'+row.ticket_number);

             } else {

              return createElement('router-link', { attrs: { to: '/thread/' + row.id }}, '#'+row.ticket_number);
            }
          }
        },
