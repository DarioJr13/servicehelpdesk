		...
		<div class="card card-light">
			...
			<div class="card-header">
				...
				<custom-field-filter :selectedFilters="customFieldFilters" v-on:custom="getCustomFields" :customFilters="filter" 
					:updatedFieldValues="getUpdatedFilter" ref="custom_filter" :showLoader="visibleLoader">
				...
				<save-filter v-if="showFilterPopUp" :filterName="filterName" :filterValues="filter" :filterObjects="filterObjects" 
					...
					:dashboard-config="dashboardConfig" :from="from" :totalFilters="totalFilters">
				...
				<template v-if="showFilter">
					...
					<template v-if="from === 'clientFilter'">
						...
						<div class="row">
							...
							<common-api-filter :label="lang('status')" :api="'statuses?client_visibility=1'" 
		  	...
		  	<div class="card-footer">
				...
				<template v-else>
					...
					<button id="edit-filter" v-if="!isApplyOnlyMode" type="button" @click = "saveFilter()" class="btn btn-primary">
						...
						<span v-else><i class="fas fa-edit" aria-hidden="true"></i>&nbsp;{{lang('edit')}}</span>
					</button>
	...
	export default {
		...
		methods:{
			...
			appendCustomFieldFilter(element){
				...
				this.$refs.custom_filter.getCustomFields(event,element,'append')
			},
		...
		'custom-form-fields': require('./CustomFields'),
	  },
	};
</script>
<style scoped>
