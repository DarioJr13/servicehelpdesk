<template>
	...
	<div class="card-tools">
		...
		<a href="javascript:;" class="btn-tool dropdown-toggle" data-toggle="dropdown" v-tooltip="trans('add_custom_field')" 
			@click.once="getCustomFields($event)">
	...
	export default{
		...
		methods:{
					...
					for(var i in data){
						...
						if(this.valuesInEdit.length){
							...
							for(var k in this.valuesInEdit){
								...
								if(this.valuesInEdit[k].key == data[i].unique){
			...
			toggleFormFieldVisibility(formFieldObject){
				...
				let index = this.allCustomFields.findIndex(obj => obj.unique == formFieldObject.unique);
				...
				if(!formFieldObject.selected){
					...
					delete  this.updatedFilterValues[formFieldObject.unique];
