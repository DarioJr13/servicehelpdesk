  ...
  <div id="main-div">
    ...
    <faveo-box :title="lang('list_of_agents')" boxClass="card-light">
      ...
      <div class="card-tools" slot="headerMenu" >
        ...
        <a class="btn btn-tool" @click="toggleFilterView" v-tooltip="lang('filter')">
        ...
        headings: {
          ...
          name: function(createElement, row) {
            let a = createElement('a', {
              attrs:{
                'href': window.axios.defaults.baseURL + '/panel/user/' + row.id,
                'target': "_blank"
              }
            }, row.name);
          ...
          if(data) {
					return {
  						...
  						data.view_url = window.axios.defaults.baseURL + '/panel/user/' + data.id ;
              data.agent_view_url = window.axios.defaults.baseURL + '/panel/agent/' + data.id ; 
              data.from = 'admin'
  						return data;
  						}),
